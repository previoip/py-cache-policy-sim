...
class RecsysFL:
  ...
  class ModelRunner:
    ...
    def __init__(self, daisy_config):
      self.wref_daisy_config = daisy_config
      self.model = None
      self.sampler = None
      self.splitter = None
    ...
    @staticmethod
    def train_proc_1(train_set):
      self.model.fit(train_set)

    @staticmethod
    def train_proc_2(train_set):
      uid = self.wref_daisy_config['UID_NAME']
      batch_size = self.wref_daisy_config['batch_size']

      history_item_id, history_item_value, _  = get_history_matrix(train_set, self.wref_daisy_config, row=uid)
      self.wref_daisy_config.update({'history_item_id': history_item_id, 'history_item_value': history_item_value})
    ...
    @staticmethod
    def train_proc_3(train_set):
      sampler = self.sampler(train_set, self.wref_daisy_config)
      batch_size = self.wref_daisy_config['batch_size']
    ...
    @staticmethod
    def train_proc_4(train_set):
      self.wref_daisy_config.update({'inter_matrix': get_inter_matrix(train_set, self.wref_daisy_config)})
      sampler = self.sampler(train_set, self.wref_daisy_config)
      batch_size = self.wref_daisy_config['batch_size']
  ...
  def new_recsys_runner(cls, daisy_config):
    ...
    model_name = runner.wref_daisy_config['algo_name'].lower()
    model_class = model_dict.get(model_name)
    model = model_class(runner.wref_daisy_config.copy())
    runner.set_model(model)

    runner.set_splitter(TestSplitter(runner.wref_daisy_config))

    if preset == 1:
      runner.set_train_procedure(cls.ModelRunner.train_proc_1)
    elif preset == 2:
      runner.set_train_procedure(cls.ModelRunner.train_proc_2)
    elif preset == 3:
      runner.set_train_procedure(cls.ModelRunner.train_proc_3)
      runner.set_sampler(BasicNegtiveSampler)
    elif preset == 4:
      runner.set_train_procedure(cls.ModelRunner.train_proc_4)
      runner.set_sampler(BasicNegtiveSampler)
    elif preset == 5:
      runner.set_train_procedure(cls.ModelRunner.train_proc_5)
